...
def learing_rate_scheduler(optimizer, config):
    if "type" in config._config["lr_scheduler"]: 
        if config["lr_scheduler"]["type"] == "CustomLR": # linear learning rate decay
            ...
            def lr_lambda(epoch):
                if "step3" in lr_scheduler_args and epoch >= lr_scheduler_args["step3"]:
                    lr = gamma * gamma * gamma
                elif epoch >= lr_scheduler_args["step2"]:
                    lr = gamma * gamma
...
def main(config):
    ...
    loader = []
    for _ in range(2):
            loader.append(config.init_obj('data_loader', module_data))
    '''
